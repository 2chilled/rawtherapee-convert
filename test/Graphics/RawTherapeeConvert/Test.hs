...
tests :: SpecWith ()
tests = describe "Graphics.RawTherapeeConvert" $ do
  describe "getTargetDirectoryPath" $ do
    it
        "should return a Left if sourceFilePath is not a subpath of root source dir and a Right otherwise"
      $ property getTargetDirectoryShouldReturnTheCorrectEither
    it "should work correctly"
      $ let
          fp        = "/bla/1/2/blubb.CR2"
          targetDir = "/aha/x/"
          result    = getTargetDirectoryPath (RootSourceDir "/bla/")
                                             (RootTargetDir targetDir)
                                             fp
          resultExpected =
            Right "/aha/x/1/2" :: Either GetTargetDirectoryException String
        in
  ...
  describe "cr2Paths"
    ...
    $ \dir -> do
        ...
        let
          toTest =
            cr2Paths testLogger dir .| CC.map (const (1 :: Sum Int)) .| CC.fold
  ...
  describe "findPp3"
    ...
    $ \dir -> do
        [f1Cr2, _, f2Cr2] <-
          sequenceA
          $   createFile dir
          <$> ["file1.CR2", "file1.CR2.pp3", "file2.CR2"]
  ...
  describe "isConversionNecessary" $ do
    it "should return true if target pp3 file is not existing" $ property
      isConversionNecessaryShouldReturnTrueIfTargetPp3FileIsNotExisting
    it
        "should return true if target pp3 file is existing but not equal to given pp3"
  ...
  describe "parseIni" $ it "should be able to parse our pp3 file" $ do
    either' <- readIniFile "test/resources/IMG_3695.CR2.pp3"
    ("" :: String) <$ either' `shouldNotBe` Left ""
  describe "extractDlnaIniEntries"
    $ it "should extract all resize ini entries"
    $ do
        (Right ini) <- readIniFile "test/resources/IMG_3695.CR2.pp3"
        let result = extractDlnaIniEntries ini
        let expected =
              [ ("Enabled"      , "false")
              , ("Scale"        , "1")
...
getTargetDirectoryShouldReturnTheCorrectEither (TestFilePath fp) (MyText anyText)
  = strip anyText
    /=  ""
    ==> let
          fpUp        = takeDirectory fp
          anyString   = unpack anyText
          fpDifferent = fpUp <> "bla"
          dirPath rootSourceDir = getTargetDirectoryPath
            (RootSourceDir
              (trace ("rootSourceDir = " <> rootSourceDir) rootSourceDir)
            )
          ...
          leftResultExpected =
            Left
              (SourceFilePathIsNotUnderRootSourceDir
                (RootSourceDir fpDifferent)
                fp
              )
          rightResult = dirPath fpUp
          rightResultExpected =
            Right anyString :: Either GetTargetDirectoryException String
        in
          property
          $  leftResult
          == leftResultExpected
...
withEmptyTmpDir f = do
 ...
 where
  handler :: FilePath -> SomeException -> IO x
  handler fp e = do
...
createSourceFile sourceDir =
  let f = sourceDir </> "test.CR2" in f <$ writeFile f ""

withSourceTargetAndSourceFile
  :: (FilePath -> FilePath -> FilePath -> IO x) -> IO x
withSourceTargetAndSourceFile f =
  withEmptySourceAndTarget $ \sourceDir targetDir ->
    createSourceFile sourceDir >>= f sourceDir targetDir
...
getEmptyTemporaryDirectory = do
  ...
  emptyDir           <- if existingFileOrDir'
    then getEmptyTemporaryDirectory
...
createFile p fp =
  let resultPath = p </> fp in resultPath <$ writeFile resultPath ""
...
isConversionNecessaryShouldReturnTrueIfTargetPp3FileIsNotExisting
  :: SourceFilePathShouldContainPp3 -> DefaultPp3ShouldBeGiven -> Property
isConversionNecessaryShouldReturnTrueIfTargetPp3FileIsNotExisting (SourceFilePathShouldContainPp3 sourceFilePathShouldContainPp3) (DefaultPp3ShouldBeGiven defaultPp3ShouldBeGiven)
  = ioProperty
    . withSourceTargetAndSourceFile
    $ \sourceDir targetDir sourceFile -> do
...
isConversionNecessaryShouldBehaveCorrectlyForAllCases
  :: SourceFilePathShouldContainPp3
...
isConversionNecessaryShouldBehaveCorrectlyForAllCases (SourceFilePathShouldContainPp3 sourceFilePathShouldContainPp3) -- true
  ...
  = ioProperty
    ...
    $ \sourceDir targetDir sourceFile ->
        let targetPp3Content =
              if sourcePp3ShouldEqualTargetPp3 then "a" else "aa"
        in
          do
            _ <- when targetFilePathShouldExist
              $ writeFile (targetDir </> "test.jpg") ""
            _ <- when sourceFilePathShouldContainPp3
              $ writeFile (sourceDir </> "test.CR2.pp3") "a"
            _ <- when targetFilePathShouldContainPp3
              $ writeFile (targetDir </> "test.jpg.pp3") targetPp3Content
            defaultPp3 <- if defaultPp3ShouldBeGiven
              then
            ...
            let shouldBeNecessary = not
                  ...
                  && targetFilePathShouldContainPp3
                  && (  sourcePp3ShouldEqualTargetPp3
                     || (  not sourceFilePathShouldContainPp3
                        && not defaultPp3ShouldBeGiven
                        )
                     )
                  )
